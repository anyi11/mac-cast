...
class SSDPServer:
    ...
    def start(self):
        ...
        if not self.running:
            self.running = True
            self.sending_byebye = True
            self.ssdp_thread = threading.Thread(target=self.run, name="SSDP_THREAD", daemon=True)
    ...
    def run(self):
        ...
        try:
            ...
            threading.Thread(target=lambda: Setting.stop_service(), name="SSDP_STOP_THREAD", daemon=True).start()
            return
