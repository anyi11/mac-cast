...
local language = {
    ["default"] = {
        ...
        ontop = "窗口置顶",
        ontop_disable = "取消置顶",
        file_loop_enable = "单片循环开启",
        file_loop_disable = "单片循环关闭",
        playlist_loop_enable = "列表循环开启",
        playlist_loop_disable = "列表循环关闭",
        shuffle = "随机播放开启",
        unshuffle = "随机播放关闭",
        speed_control = "播放速度",
        screenshot = "截图",
...
local function load_locale_file()
    ...
    local locale_path = script_dir and (script_dir .. "/modernz-locale.json") or "~~/script-opts/modernz-locale.json"
...
layouts["compact"] = function ()
    ...
    elements["play_pause"].visible = osc_param.playresx >= 0
    lo = add_layout("play_pause")
    ...
    if user_opts.track_nextprev_buttons then
        local prev_vis = osc_param.playresx >= 450
        ...
        local next_vis = osc_param.playresx >= 400
    ...
    if state.audio_track_count > 0 and user_opts.volume_control then
        left_side_button("vol_ctrl", 480, nil, 20)

        local vol_vis = osc_param.playresx >= 480
        ne = new_element("volumebarbg", "box")
        ne.visible = vol_vis
        elements["volumebar"].visible = vol_vis
        if vol_vis then
    ...
    right_side_button("fullscreen", 0, user_opts.fullscreen_button)
    ...
    right_side_button("playlist", 500, user_opts.playlist_button)
    ...
    -- time codes
    local time_codes_width = get_time_codes_width()
    elements["time_codes"].visible = (state.duration or 0) > 0 and osc_param.playresx >= 360
    lo = add_layout("time_codes")
...
local function osc_init()
    ...
    ne.enabled = (mp.get_property("user-data/macast/has-prev", "no") == "yes") or (pl_pos > 1) or (loop ~= "no") or contains(user_opts.buttons_always_active, "playlist_prev")
    bind_buttons("playlist_prev")

    --next
    ne = new_element("playlist_next", "button")
    ne.content = icons.next
    ne.enabled = (mp.get_property("user-data/macast/has-next", "no") == "yes") or (have_pl and (pl_pos < pl_count)) or (loop ~= "no") or contains(user_opts.buttons_always_active, "playlist_next")
    ...
    ne.slider.posF = function ()
        if ne.state.mbtnleft and ne.state.drag_pos then
            return ne.state.drag_pos
        end
    ...
    ne.eventresponder["mouse_move"] = function (element)
        ...
        element.state.drag_pos = seekto
        request_tick()
    end
    ne.eventresponder["mbtn_left_down"] = function (element)
        element.state.mbtnleft = true
        element.state.was_paused = mp.get_property_bool("pause")
        element.state.drag_pos = get_slider_value(element)
        state.playing_and_seeking = false
        request_tick()
    end
    ...
    ne.eventresponder["mbtn_left_up"] = function (element)
        ...
        if state.playing_and_seeking then
            local seekto = element.state.drag_pos or get_slider_value(element)
