...
## 🌟 v1.2.6 最新更新亮点 (2026-07-14)

1. **网络连接稳定性 (VPN/Clash 兼容)**：绑定各物理网卡的专属套接字（Socket），解决 macOS 开启 Clash 等 VPN（TUN 模式）后，因默认路由被代理网卡接管导致手机端无法搜索到设备或经常断连的问题。
2. **播放器自动休眠回收**：解决视频播放完毕后黑色窗口长期挂载在桌面的问题。现增加 5 秒休眠缓冲，若无后续切歌则自动优雅清退并关闭播放器。
3. **Bilibili 协议性能优化**：修复 Bilibili 投屏协议存在的互斥锁高占有卡死问题，彻底根治开启 Bilibili 协议后软件频繁假死、不响应投屏的问题。
4. **原生 Apple Silicon (arm64) 极简版**：
   - 彻底移除了 Intel (x86_64) 二进制，内置播放器和依赖库全线升级为原生 **Apple Silicon (arm64)**，大幅缩减内存和 CPU 转译损耗。
   - 对所有 `.so` 二进制模块进行 `lipo` 裁剪与 `strip` 符号脱水。
   - **App 体积从 97MB 降至 54MB，发布包压缩至 24MB**，轻量且强大。
5. **后台日志与投屏历史监控**：
   - 将日志监控移至 App 启动时，使状态栏的投屏历史不打开日志面板也能实时同步更新。
   - 适配 `macast.log` 大小轮转，自动拼合 `macast.log.1` 的历史数据，彻底解决投屏记录滚动时自动消失的问题。
...
| **应用体积瘦身** | 应用 97MB / 发布包 41MB | 应用 **54MB** / 发布包 **24MB** | 原生 arm64，剔除无用架构与符号，体积减小 44%，内存开销更低 |
